/*
 * Demostrate a trivial filesystem using libfs.
 *
...
static int lfs_dir_create_file(struct inode *inode, struct dentry *dentry,
                               int flags, struct nameidata* nd) {
    printk(KERN_INFO "Creating %lxd %lxd %i %lxd", (unsigned long)inode,
           (unsigned long)dentry, flags, (unsigned long)nd);
    return 0;
}
...
static struct inode_operations lfs_dir_inode_ops = {
    .lookup = simple_lookup,
    .create = lfs_dir_create_file,
};
